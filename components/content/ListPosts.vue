...
watch(
  ...
  () => {
    if (route.query.tags) {
      query.value.where = {
        // @ts-expect-error expected to type have a value
        type: "post",
        tags: {
    ...
    <ul class="grid grid-cols-1 md:grid-cols-2 gap-10">
      <ContentList v-slot="{ list }" :query="query">
        <li v-for="post in list" :key="post._path" class="before:hidden !pl-0">
          <NuxtLink
            class="block w-full h-full bg-secondary-800 rounded-md overflow-hidden flex flex-col group"
            :to="post._path"
          >
            ...
            <div class="px-8 py-6 flex flex-col justify-between flex-grow">
              <div>
                <div
                  ...
                  class="text-sm mb-2 text-cool-gray-500 font-bold"
                >
                  {{ post.categories[0] }}
                </div>
